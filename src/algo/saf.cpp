...
void SAF::on_simulation_start(double date,
...
void SAF::on_simulation_end(double date)
{
    (void) date;
}

void SAF::make_decisions(double date,
    ...
    SortableJobOrder::CompareInformation *compare_info)
{
    ...
    // This algorithm is a version of SAF without backfilling.
    // It is meant to be fast in the usual case, not to handle corner cases.
    // It is not meant to be easily readable or hackable ;).

    // This fast SAF variant in a few words:
    // - only handles the SAF queue order
